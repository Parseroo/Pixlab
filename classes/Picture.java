...
public class Picture extends SimplePicture {
  ...
  public void myCollage() {
    ...
    motor.keepOnlyRed();
    this.copy(motor, 300, 0);
    this.copy(beach, 500, 6);
    Picture beach2 = new Picture("images/beach.jpg");
    beach2.mirrorHorizontalBotToTop();
    this.copy(beach2, 275, 300);
  ...
  public void mirrorArms() {
    ...
    Pixel topPixel = null;
    Pixel bottomPixel = null;
    for(int i = 104; i < 170; i++) {
      for(int j = 155; j<191; j++) {
        topPixel = pixels[j][i];
        bottomPixel = pixels[191-j+191][i];
        bottomPixel.setColor(topPixel.getColor());
      }
    }
    for(int i = 238; i < 295; i++) {
      for(int j = 170; j<195; j++) {
        topPixel = pixels[j][i];
        bottomPixel = pixels[190-j+190][i];
        bottomPixel.setColor(topPixel.getColor());
      }
    }
  }
